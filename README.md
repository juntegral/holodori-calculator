...
## 📐 2026-08-05 업데이트: 판정/콤보/발동률/스코어레이트 산출식 발견 (비공식, 부분)

**공식(COVER/QualiArts) 스코어 계산식은 여전히 어디에도 공개되지 않았습니다.** 다만
2026-08-05 정기 점검에서, horodori.com보다 훨씬 구체적인 수치를 공개한 비공식 팬사이트
**dreams.wf-calc.net**("ホロドリ研究室"/Holodori Lab)을 새로 발견했습니다:
...
>
> — [dreams.wf-calc.net/wiki/live](https://dreams.wf-calc.net/wiki/live?lang=ja) (2026-08-05 확인)

이 사이트도 스스로를 "비공식 데이터베이스＆도구 사이트"라고만 밝힐 뿐 데이터마인인지
실측 테스트인지는 명시하지 않으므로, horodori.com과 마찬가지로 **팬 추정치**로 취급해야
합니다. 다만 "미검증 추측"과 "확정 수치"를 사이트 스스로 구분해 표기하고 있어(예:
노트 종류별 스코어 차이는 없다고 명시하면서도, 난이도별 만점 동일 여부는 별도로
"미검증"이라고 표기) 신뢰도가 horodori.com보다는 다소 높다고 판단됩니다.

src/calculator.js에 위 공식을 그대로 구현한 **독립 유틸리티 함수**를 추가했습니다:
calcNoteScore, calcComboBonusPct, calcActiveSkillRate, calcSongScoreRate,
calcHolomemScoreRate, JUDGMENT_SCORE_RATIO, ACTIVE_SKILL_BASE_RATE_PCT. 기존
calcUnitStats()/listSkills()의 동작과 API는 **전혀 바뀌지 않았습니다** — 이
신규 함수들은 아직 data/members.json의 카드별 수치와 자동으로 연결되어 있지 않고
(스코어UP과 파라미터UP의 스키마 구분이 아직 불명확), 값을 직접 넣어 계산하는 수동
유틸리티입니다.
...
## 구조: 리더 1 + 멤버 카드 5장 (2026-07-29 사용자 확인, 2차 정정 포함)

- **확정된 구조**: 리더는 "캐릭터+의상" 슬롯이고 멤버 5장은 카드 슬롯으로 **서로 독립**입니다.
  **리더와 같은 캐릭터의 카드를 멤버 5장 안에 넣는 것도 가능**합니다(2026-07-29 사용자
  실편성으로 확인: 노엘 리더 + 멤버에도 노엘 카드). 리더 슬롯 자체는 스탯/조건 카운트에
  기여하지 않지만, 같은 캐릭터의 카드가 멤버로 들어가 있으면 그 멤버 카드는 정상
  카운트됩니다. 리더의 의상스킬 발동조건(예:
...
// 예: 라플라스를 리더로, 멤버 5장을 시산 (리더와 같은 캐릭터의 카드를 멤버에 넣는 것도 가능)
...
2. **스코어서포트/스페셜/패시브·액티브 효과량은 계산에 전혀 반영되지 않음** — 이는
   ...
   확인 가능.
   (2026-08-05 업데이트: dreams.wf-calc.net 발 판정/콤보/발동률/스코어레이트 공식은
   `calcNoteScore`/`calcComboBonusPct`/`calcActiveSkillRate`/`calcSongScoreRate`/
   `calcHolomemScoreRate` 유틸리티 함수로 별도 구현했으나, `calcUnitStats()`에는 아직
   자동 연결되지 않음 — 위 "2026-08-05 업데이트" 섹션 참고.)
